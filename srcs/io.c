...
void print_usage(char *arg, char opt) {
	...
	fprintf(stderr, "  -s <size>   Set packet size to <size> bytes\n");
	fprintf(stderr, "  -l <preload> Preload <preload> packets before starting\n");
	fprintf(stderr, "  -W <timeout> Set timeout for each packet in seconds\n");
}